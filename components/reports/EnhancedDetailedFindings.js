...
export default function EnhancedDetailedFindings({ sections = [], observations = [] }) {
  ...
  // Helper function to check if video is archived (older than 90 days)
  const isVideoArchived = (section) => {
  ...
  // Helper function to format the archived date
  ...
  const handleVideoPlay = (section, timecode) => {
    ...
    setVideoModal({
      ...
      pauseTime: timecode, // Store the exact timestamp to pause at
      sectionName: section.name,
      isArchived: isVideoArchived(section),
      createdAt: section.created_at
    })
  }

  // Handle video time updates to pause at the exact timestamp
  const handleVideoTimeUpdate = (e) => {
    if (videoModal && videoModal.pauseTime !== undefined) {
      const currentTime = e.target.currentTime
      const pauseTime = videoModal.pauseTime
      
      // Check if we've reached or passed the pause time (with small tolerance for precision)
      if (currentTime >= pauseTime && !e.target.paused) {
        e.target.currentTime = pauseTime // Set exact time
        e.target.pause() // Pause the video
        
        // Clear the pause time so we don't keep pausing on subsequent time updates
        setVideoModal(prev => ({
          ...prev,
          pauseTime: undefined
        }))
                          ...
                          <div className="flex items-start space-x-4">
                            ...
                            {obs.video_timestamp !== undefined && section.video_url && (
                              <button
                                ...
                                className={`flex items-center text-sm no-print ${
                                  isVideoArchived(section) 
                                    ? 'text-gray-500 cursor-not-allowed' 
                                    : 'text-blue-600 hover:text-blue-700'
                                }`}
                                title={
                                  isVideoArchived(section)
                                    ? `Video archived (created ${formatArchivedDate(section.created_at)})`
                                    : "Play video from this observation"
                                }
                              >
      ...
      {videoModal && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4 no-print">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="p-4">
              {videoModal.isArchived ? (
                // Archived video message
                <div className="text-center py-12">
                  <div className="mx-auto w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                    <Package className="w-8 h-8 text-gray-400" />
                  </div>
                  <h3 className="text-lg font-medium text-gray-900 mb-2">Video Archived</h3>
                  <p className="text-gray-600 mb-4">
                    This video was created on {formatArchivedDate(videoModal.createdAt)} and has been 
                    moved to long-term storage after 90 days.
                  </p>
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-left">
                    <p className="text-sm text-blue-800">
                      <strong>Why is this video archived?</strong><br />
                      Videos older than 90 days are automatically moved to cost-effective long-term storage 
                      to reduce hosting costs while preserving the inspection data.
                    </p>
                  </div>
                </div>
              ) : (
